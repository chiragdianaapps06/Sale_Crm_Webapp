...
class VerifyOTP(APIView):
    def post(self, request):
        try:
            ...
            # Optional: Only check for password fields if they're present
            password = request.data.get('password', '')
            ...
            # Handling Forgot Password (When password and confirm_password are provided)
            if password and confirm_password:
                ...
                # Handling Registration (Only email and OTP required)
                try:
                    # Create user for registration
                    user = CustomUser.objects.get(email=email)  # Check if user already exists
                    logging.warning(f"User with email {email} already exists. Skipping registration.")
                    return Response({
